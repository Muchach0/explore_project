...
## Time in seconds the player must channel before the skill fires (0 = instant)
@export var cast_time: float = 0.0

@export var is_hot_or_dot: bool = false
...
func level_up() -> void:
    level += 1
    print("Skill.gd - level_up() - Skill level up: %s, new level: %d" % [skill_name, level])
...
# func get_direction_to_target(player: Player, target_position: Vector3) -> Vector3:
