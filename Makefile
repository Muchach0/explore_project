...
DOCKER_IMAGE_CLIENT = muchachoo/tomatoe-heal
# DOCKER_IMAGE_SERVER = muchachoo/tomatoe-mmo-server
DOCKER_IMAGE_TAG = 1
GCP_CLOUD_RUN_SERVICE_CLIENT = tomatoe-heal-client
# GCP_CLOUD_RUN_SERVICE_SERVER = tomatoe-mmo-server
DNS_ZONE = muchacho-app
DNS_NAME_CLIENT = tomatoe-heal.muchacho.app
...
export-linux:
	$(GODOT_BINARY) --headless --path $(shell pwd) --export-release "Linux" $(shell pwd)/builds/linux/$(LINUX_BINARY)
